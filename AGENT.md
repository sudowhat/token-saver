...
Token Saver is an **optimization layer** for a software project using AI tools. It does not replace the target project's own agent protocol/skills, architecture, build rules, tests, or existing versioning system workflow.

## Initialization

When asked to initialize a project:

1. Identify the target repository/project root.
...
4. Inspect only enough repository structure to understand the current task and relevant build/test entry points.
...
3. authoritative target-project instructions and current live repository state;
4. exact source code, tests, build configuration, Git state, and current documentation;
...
Do not require any one AI vendor, IDE, MCP host, hook system, plugin, or external tool.

Vendor-specific adapters are optional convenience only. The canonical behavior lives in this file and the five skills.
